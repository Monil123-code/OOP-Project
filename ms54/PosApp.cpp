...
namespace sdds
{
	...
	int PosApp::selectItem()
	{
		...
		int input = 0;
		int result = integerInput("Enter the row number: ", input, nptr);
		return result;
	}
	...
	void PosApp::stockItem() {
		...
		Item* itemToAdd = Iptr[row - 1]; //creating pointer to item

		cout << "Selected Item:"<<endl;

		itemToAdd->displayType(POS_FORM); // setting the displayType
		itemToAdd->write(cout); // calling the write function to print the item
		cout << "Enter quantity to add: ";
		int input = 0;
		int quantity = integerInput("Enter quantity to add: ", input, 69); // getting fool-proof integer input
		itemToAdd->operator+=(quantity); // adding the quantity to the selected item 
		cout << ">>>> DONE!..................................................................." << endl;
	}
	...
	int  PosApp::integerInput(const std::string& prompt,int& input, const int maxValue)
	{
		...
		if (cin.fail())
		{
			...
			cin.ignore(1000, '\n');
			cout << "Invalid Integer, try again: ";
			integerInput(prompt ,input ,maxValue);
		}
		else if (input < 1 || input > maxValue) {
			cout << "[1<=value<=" << maxValue << "], retry: "<< prompt;
			integerInput(prompt,input,maxValue);
		}
		return input;
	}
